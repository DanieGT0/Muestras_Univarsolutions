...
import { Plus, Edit2, Trash2, Truck, CheckCircle, BarChart3, Globe, Upload } from 'lucide-react';
...
import { suppliersAPI, countriesAPI } from '../../lib/api';
...
export function SuppliersManagement() {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const [suppliersResponse, countriesResponse] = await Promise.all([
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: 'Error',
        description: `No se pudo ${editingSupplier ? 'actualizar' : 'crear'} el proveedor`,
        variant: 'destructive',
      });
    }
  };
  ...
  const handleEdit = (supplier: Supplier) => {
    ...
    setFormData({
      cod: supplier.cod,
      name: supplier.name,
      country_ids: supplier.countries?.map(c => c.id) || []
    });
    setShowForm(true);
  ...
  const lastCreatedCode = suppliers.length > 0 ? suppliers[suppliers.length - 1].cod : 'N/A';
        ...
        <Card className="p-4 bg-white border-slate-200 hover:shadow-lg transition-all duration-200 border-l-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-slate-700">{suppliers.length}</p>
            </div>
          </div>
        </Card>
      ...
      {showForm && (
        <Card ref={formRef} className="p-6 border-slate-200 bg-slate-50">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {!editingSupplier && suppliers.length > 0 && (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder="Proveedor Internacional S.A."
                  required
                />
              </div>
            </div>
        ...
        <div className="flex items-center justify-between p-4 border-t">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-gray-600">
              {startIndex + 1}-{Math.min(endIndex, suppliers.length)} de {suppliers.length}
            </span>
          </div>
